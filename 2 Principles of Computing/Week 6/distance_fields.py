...
def create_distance_field(entity_list):
    ...
    """
    # Create empty distance field
    distance_field = [[0 for dummy_col in range(GRID_WIDTH)]
                      for dummy_row in range(GRID_HEIGHT)]

    # Calculcate the distances
    for row in range(GRID_HEIGHT):
        for col in range(GRID_WIDTH):
            distance = min(
                [manhattan_distance(entity[0], entity[1], row, col) for entity in entity_list])
